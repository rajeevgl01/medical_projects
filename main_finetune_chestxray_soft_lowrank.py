...
from jointModel import LowRankLinear, Classifier, JointLowRankModel

from engine_finetune_soft import train_one_epoch, evaluate_chestxray, evaluate_chestxray_before
...
def make_plots(plot_list, plot_info):
	...
	plt.grid(True)
	plt.tight_layout()
	plt.savefig(os.path.join(args.output_dir, f"{plot_info}_{args.tn_loss_factor}.png"))
	plt.close()
...
def get_args_parser():
	...
	parser.add_argument('--softRank', type=int, default=76,
						help='low rank')
	parser.add_argument('--trainLinearLayer', action='store_true', default=False,
						help='for training/finetuning linear mapping')
	parser.add_argument('--train_all', action='store_true', default=False,
						help='for training/finetuning base model')
	parser.add_argument('--soft_low_rank', action='store_true', default=False,
						help='for soft low rank')
	...
	parser.add_argument('--get_features', action='store_true',
						help='Get features before classification head')
	...
	parser.add_argument("--dataset_train", default='chestxray_soft', type=str)
	parser.add_argument("--dataset_test", default='chestxray', type=str)
...
def main(args):
	...
	logger('job dir: {}'.format(os.path.dirname(os.path.realpath(__file__))))
	logger("{}".format(args).replace(', ', ',\n'))
	...
	dataset_test = build_dataset_chest_xray(split='test', args=args, test=True)
	...
	if True:  # args.distributed:
		...
		logger("Sampler_train = %s" % str(sampler_train))
		if args.dist_eval:
			if len(dataset_test) % num_tasks != 0:
				logger('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
	...
	if 'vit' in args.model:
		model = models_vit.__dict__[args.model](
			...
			soft_low_rank=args.soft_low_rank
		)

	elif 'densenet' in args.model:
		model = models_densenet.__dict__[args.model](
			num_classes=args.nb_classes, get_features=args.get_features, soft_low_rank=args.soft_low_rank)
	...
	if args.finetune:
		if 'vit' in args.model:
			...
			logger("Load pre-trained checkpoint from: %s" % args.finetune)
			checkpoint_model = checkpoint['model']
			state_dict = model.state_dict()
			for k in ['head.weight', 'head.bias']:
				if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
					logger(f"Removing key {k} from pretrained checkpoint")
	...
	if args.retrain:
		model.load_state_dict(torch.load(args.retrain)['model'])

	model.to(device)

	if args.soft_low_rank:
	...
	logger("Model = %s" % str(model_without_ddp))
	logger('number of params (M): %.2f' % (n_parameters / 1e6))
	...
	logger("base lr: %.2e" % (args.lr * 256 / eff_batch_size))
	logger("actual lr: %.2e" % args.lr)

	logger("accumulate grad iterations: %d" % args.accum_iter)
	logger("effective batch size: %d" % eff_batch_size)
	...
	if args.dataset_train == 'chestxray_soft':
	...
	logger("criterion = %s" % str(criterion))

	misc.load_model(args=args, model_without_ddp=model_without_ddp,
					optimizer=optimizer, loss_scaler=loss_scaler)

	logger(f"Start training for {args.epochs} epochs")
	...
	for epoch in range(args.start_epoch, args.epochs):
		...
		# #U, S, V = torch.empty((0)).cuda().contiguous(), torch.empty(
		# #    (0)).cuda().contiguous(), torch.empty((0)).cuda().contiguous()
		...
		new_features = features[sorted_indexes]

		new_features = new_features.cpu().numpy()

		U, S, V = scipy.linalg.svd(new_features, full_matrices=False)
		U, S, V = torch.tensor(U).cuda().contiguous(), torch.tensor(S).cuda().contiguous(), torch.tensor(V).cuda().contiguous()

		# else:
		# 	U, S, V = torch.empty(1).cuda(), torch.empty(1).cuda(), torch.empty(1).cuda()
			#U = dist.broadcast(U, src=0)
		# group = dist.new_group(list(range(4)))
		
		# dist.barrier()
		# dist.broadcast(U, src=0, group=dist.group.WORLD)
		# dist.broadcast(S, src=0, group=dist.group.WORLD)
		# dist.broadcast(V, src=0, group=dist.group.WORLD)

		#U = dist.broadcast(U, src=0)
		#S = dist.broadcast(S, src=0)
		#V = dist.broadcast(V, src=0)
		...
		if args.output_dir and (epoch % args.eval_interval == 0 or epoch + 1 == args.epochs):
			misc.save_model(args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
							loss_scaler=loss_scaler, epoch=epoch, name=f'latest_{args.softRank}_{args.tn_loss_factor}')

			test_stats, loss = evaluate_chestxray(
			...
			logger(f'Max Average AUC: {max_auc:.4f}')

			if loss < best_loss:
				best_loss = loss
				misc.save_model(args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
								loss_scaler=loss_scaler, epoch=epoch, name=f'best_loss_{args.softRank}_{args.tn_loss_factor}')

			if best_auc <= test_stats['auc_avg']:
				best_auc = test_stats['auc_avg']
				misc.save_model(args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
								loss_scaler=loss_scaler, epoch=epoch, name=f'best_auc_{args.softRank}_{args.tn_loss_factor}')

			# if args.dataset == 'covidx':
			# 	logger(
			# 		f"Accuracy of the network on the {len(data_loader_test)} test images: {test_stats['acc1']:.1f}%")
			# 	max_accuracy = max(max_accuracy, test_stats["acc1"])
			# 	logger(f'Max accuracy: {max_accuracy:.2f}%')
			...
			if args.output_dir and misc.is_main_process():
				if log_writer is not None:
					log_writer.flush()
				with open(os.path.join(args.output_dir, f"log_{args.softRank}_{args.tn_loss_factor}.txt"), mode="a", encoding="utf-8") as f:
